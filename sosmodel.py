...
class Observables( object ):
    ...
    def __init__(self, N, coupling_time, total_time):
        ...
        self.timepoints = np.zeros(int(round(self.total_time / self.coupling_time)) + 1, 'double')           
        
        self.roughness = np.zeros(int(round(self.total_time / self.coupling_time)) + 1, 'float') 
        
        self.thickness = np.zeros(int(round(self.total_time / self.coupling_time)) + 1, 'double') 
        self.thickness[0] = 1.0 # initial thickness is 1 (only one layer is deposited initially)
    ...
    def calculate_observables(self, surfacemat, counter):
        ...
        # store the simulation time point
        ...
        # factor 0.1 is there because in sosmain.py I'm calculating observables every 10 coupling steps, not every 1
        self.growthrate[counter] = 0.1 * np.sum(surfacemat - self.surfacemat_previous) * self.Nsq_inv_coupling_time_inv
...
def run_simu(N,dt,tf):
    ...
    gaslayer = GasLayer()
    observables = Observables(thinfilm.N, dt, tf) # provide N, the coupling time and the total time
    ...
    produce_output_counter = 0 # index for when to calculate observables
    ...
    while observables.current_time < observables.total_time_minus_1:
        ...
        if thinfilm.dtkmc < observables.coupling_time:
            ...
            # update the counter for calculating observables
            produce_output_counter += 1
            
            if produce_output_counter == 10:  # don't calculate observables on every coupling step
                ...
                observables.surfacemat_previous = thinfilm.surfacemat.copy()            
                
                # update the index for the output arrays
                counter += 1
                
                # reset 
                produce_output_counter = 0
    ...
    # trim trailing zeros from arrays in "observables" object
    observables.timepoints = np.trim_zeros( observables.timepoints, 'b' )
    observables.roughness = np.trim_zeros( observables.roughness, 'b' )
    observables.thickness = np.trim_zeros( observables.thickness, 'b' )
    observables.growthrate = np.trim_zeros( observables.growthrate, 'b' )    
